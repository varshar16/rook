...
#
PASSWD=$(kubectl -n $K8S_NAMESPACE get secret rook-ceph-dashboard-password -o yaml | grep "password:" | awk 'NR==1 {print $2}' | base64 --decode)
...
PD=$(kubectl -n $K8S_NAMESPACE get secret rook-ceph-dashboard-password -o yaml | grep "password:" | awk '{print $2}')
echo "Without base $PD"
echo "PASSWD: $PASSWD"
